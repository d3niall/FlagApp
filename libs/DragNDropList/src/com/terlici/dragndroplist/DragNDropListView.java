	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			case MotionEvent.ACTION_DOWN:
                ...
                if (mStartPosition != INVALID_POSITION && isDrag(ev)) {
                    mChildPosition = mStartPosition - getFirstVisiblePosition();
                    handlerLongPress.postDelayed(mLongPressedRunnable, 300);
                    mDragPointOffset = y - getChildAt(mChildPosition).getTop();
...
//				}
			...
			case MotionEvent.ACTION_MOVE:
                Log.e("test", "ACTION_MOVE! dragmode " + mDragMode);
			    if (mDragMode) {
                    if (mStartPosition != pointToPosition(x, y))
                        handlerLongPress.removeCallbacks(mLongPressedRunnable);
                    drag(0, y);
                }
				break;
			...
			default:
                Log.e("test", "ACTION_UP! dragmode " + mDragMode);
    ...
    Runnable mLongPressedRunnable = new Runnable() {
        public void run() {
            Log.i("test", "Long press!");
            mDragMode = true;
            Vibrator vibrator = (Vibrator) getContext().getSystemService(Context.VIBRATOR_SERVICE);
            vibrator.vibrate(100);
            startDrag(mChildPosition,0);
        }
    };
}
